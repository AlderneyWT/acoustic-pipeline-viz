      ...
      for(ii in 1:ceiling(length(potential_days)/timeFacets)){
        ...
        for(j in 1:timeFacets){
          ...
          if(!is.na(this_survey_date)){
            ...
            if(params$params_list$includeDay == T){
              these_xlims <- c(as.POSIXct(paste(this_survey_date, '12:00:00')), 
                               as.POSIXct(paste((this_survey_date+1), '12:00:00'))
              )
            }else{
              these_xlims <- 
                c(
            ...
            if(params$params_list$includeDay == T){
              these_xlims <- c(as.POSIXct(paste(as.Date(sun_toplot$datetime_start[1]), '12:00:00')), 
                               as.POSIXct(paste((as.Date(sun_toplot$datetime_end[1])), '12:00:00'))
              )
            }else{
              these_xlims <- 
                c(
